...
wget --progress=bar:force:noscroll https://repo.anaconda.com/archive/Anaconda3-2022.10-Linux-x86_64.sh
...
/opt/anaconda3/bin/conda install -y numpy scipy matplotlib pandas seaborn statsmodels pandoc

# install pandoc / markdown using anaconda3
# combine these, to speed things up
...
# this takes a while and probably not necessary most of the time, defer to doing
# install when needed instead for classes/students
#apt install -qy ffmpeg graphviz

# set up link to /etc/profile.d so standard .bashrc will set up path for all users
